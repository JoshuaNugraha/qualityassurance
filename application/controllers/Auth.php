...
class Auth extends CI_Controller
{
	...
	*/
	...
	function simpan_user(){
        ...
        if($aksi == '1'){ // add new user
             $sql = $this->db->query("SELECT * from users where email = '$email'  or username = '$username' ")->num_rows();
                if($sql > 0){
                    echo json_encode(["status" => false, "msg" => "Email atau Username sudah digunakan!"]);
                    die();
                }
                ...
                $query 		= $this->ion_auth->register($username, $password, $email, $data, $groups); 
        }else{
            // edit user
        ...
        if($query){
            ...
            echo json_encode(["status" => false, "msg" => "Maaf terjadi kesalahan"]);
        }
